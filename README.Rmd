...
2. The Pachyderm Hub interface will give you the instructions under the `connect` option 
...
4. We will work in the `spatial_demo` directory and the following commands will assume that your are there.
...
7. The key component of the pipeline is the container image that runs the code we need. For this demo we made the container public - you will need to setup some additional permissions in a production environment. 

8. Lets put the first pipeline to work
...
Yes, we can ask for a list of datums and check their status. If you do this on a command line you can `grep` (search) through the list to narrow it down.
